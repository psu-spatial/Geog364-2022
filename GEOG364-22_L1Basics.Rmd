...
Leave a blank line, and create a new heading called Module 2 Exercises. Leave a blank line afterwards too.

We’re going to work with a table of data that’s already pre-loaded into R inside the dplyr package (part of the [tidyverse](https://www.tidyverse.org/)). This dataset contains 87 rows, each one a starwars character from the [Star Wars API](https://swapi.dev/).

Make sure you have run the library code chunk.  Now, type the ?starwars command in the console. This will bring up the help file. If it can’t find the dataset, you need to load the tidyverse-package using the library command or install either tidyverse or dplyr.

```{r, eval=FALSE,include=FALSE}
#https://cran.r-project.org/web/packages/dplyr/vignettes/dplyr.html
```

Now look at the data itself. In the **CONSOLE**, type View(starwars). This will open a new tab with the spreadsheet itself.  DO NOT PUT THE VIEW COMMAND IN A CODE CHUNK (it messes up you making your final file)
...
Now look at the data itself.  In the console, type `View(mpg)`.  This will open a new tab with the spreadsheet itself.  Also look at the summary statistics. Leave a blank line and create a new code chunk containing the following code
...
```{r, echo=FALSE}
rubric <- readxl::read_excel("GEOG364_22_LRubric.xlsx")
knitr::kable(rubric) %>%   
  kable_classic_2() %>%
