...
### You 'speak' R in a CONSOLE {.unnumbered}

You can "have a conversation" with your computer in the R-Langauge using
a Console. There are many different ways you can access this, described
later on.

```{r, Tut_Fig1, echo=FALSE, fig.cap = "*The basic R console. You write in blue, the computer replies in black. The > means it is waiting for a command*",fig.align='center',out.width="80%"}
...
Just like English has changed since Shakesperean times, the R language
is always evolving. We keep track using Versions. For example, in the
screenshot above, at the top of the console you can see it was made with
`R version 4.1.0 Camp Pontanzen`

*Because there are some important new spatial commands we will be using,
we need the most up-to-date version of the R commands.*
...
As long as yours is above that you should be fine. The current most
recent version of the language on the website is:
...
These days there are MANY ways of accessing R-Studio.  

To make it easy to switch between them, we are going to be using "Projects". 
These are self contained folders that contain all of your code, data and output.  This makes it really easy to run R-studio and to move things around on your computer.  (e.g. if you move the whole folder, it all still runs).

For this course, I suggest one of two main options.  Each of the sections below is dedicated to getting up and started.
...
## R-Studio CLOUD (website)  {.tabset .tabset-fade}

### About {-}

There are many ways to access R-Studio.  The easiest is to simply to use R Online at the R-Studio Cloud website

Here you can log into an online version of R. and I believe is free for the first 25hrs each month.  Like Jupiter Notebook, you can also programme in other langauges.
...
### Creating projects/the Workspace {-} 


[![Rproject](./Figures/Tut_Fig7_ProjectVideo.png)](https://www.linkedin.com/learning/learning-the-r-tidyverse/why-should-you-use-projects-in-rstudio?u=76811570 "Why use R Projects")

An R-project is a folder that will store everything to do with each lab in one place on your computer.  This is incredibly useful - it means that if you switch from R-Cloud, to the lab computers, to your laptop, all you have to do is to move the folder and everything will just work. Learn more here. 

You will have a project for each Lab and I may share projects with example code.
...
You will need to submit two files for each lab, an ".Rmd" and a ".html".  More later on those, but here is how to download them so you can submit them
 ...
 3.  On the R-Studio Cloud website; inside your project, go to the files tab (next to Projects/Help in one quadrant)
 4.Click the checkbox by the .RmD , click the cog-wheel and then export. Repeat for the html.  (you can click, both it will download as a zip file). 
...
If you have your own computer, you can run R-Studio for free on there.  
   ...
   + Advantages:
       + Its really powerful and we will learn how to use and update it
...
### Downloading R/R-Studio on a PC: {-}
...
### Downloading R/R-Studio on a Mac: {-}
...
# Initial set-up  {.tabset .tabset-fade}
...
## Changing settings (LAB 1 IMPORTANT)
...
# R-Projects (one for each lab) {.tabset .tabset-fade} 
...
## Using Projects on R-Studio Cloud{-}

These are literally built in when you create a new project from the workspace.

For each new lab, go back to the workspace and create a new one
(see above in the cloud section)
...
1. IMPORTANT!  If you haven't already, on an easy to access place on your computer, make a folder called GEOG-364.  This is where ALL your labs are going to live. 
...
BELOW THIS POINT I HAVE NOT HAD THE TIME TO MAKE THE TUTORIALS PRETTY
...
Now we are going to download most of the packages you need for these labs so that you do not need to do this each week. We're doing it in groups of 5 to make it easier to check for errors.

**Copy each of these commands into the R-CONSOLE and press enter to run.**  

A load of text should stream down your console.  Just let it run until the console is back to the > symbol and you can type. It might take several minutes.  Sometimes this will give you an error and not run, 

**WHAT DOES SUCCESS LOOK LIKE?**

If it works, you should see some text like "the downloaded binary packages are in....".  If not, there will be errors.
...
Sometimes you need to install a package from a different place, like github. To do this we use a more complex command from the remotes library.  Sometimes when doing so, R will ask questions IN THE CONSOLE.  It will wait for your answer until you type it and press enter!  Choose to install all sub-packages.
...
# Console vs scripts vs Markdown {.tabset .tabset-fade}
...
Remember that the aim of programming is to provide a language so you can ask your computer to do complex tasks. The console window (see Figure \@ref(fig:Tut_Fig1)) is like a phone call with your computer, where you "speak" in R.  
...
Let's start by the simplest command possible.  
...
<p class="comment">WE WILL NOT USE THIS OPTION, but it's useful for a lot of programming and zero writing</p>
...
A basic script (the filetype is .r).   

 - This is simply just a blank notepad where you can save code commands. (How I learned to code)
 - You do NOT save any output

When you "run" the commands in the script, R simply copy/pastes the commands over to the console. You run the commands by highlighting (triple click to highlight a whole line), then press control-enter (or command-enter on a mac).  You can also copy/paste them accoss.

To open this you literally go to file, then new script.  
...
# R basics: Variables, text and commands

<br>

# Getting help {.tabset .tabset-fade}

<p class="comment">When programming in R, someone often tells you about a "cool new command". I want you to feel comfortable working out how to use them. The R help files are also wonderful.</p>

First, take a look here for all about help.  https://www.r-project.org/help.html 

5. Inside every R help file you will see this structure: https://bcgov.github.io/ds-cop-intro-to-r/seeking-help-in-r.html
...
Here's how I approach it, but you might find a better approach. 

1. If you don't already know, find the formal name and package name of the command

  + The only info we NEED to know about a new command is its name and the library/package/app it is stored in. <br>   
  + If you don't know, you can often google the command and look on the top left for the package and formal name. You often also find easy online tutorials..  <br>
  + Or go to https://www.rdocumentation.org/ <br>
...
```{r, tut4fig2, echo=FALSE,fig.align='center',fig.cap="Yours will say GEOG-364 not GEOG-364.."}
...
Follow Tutorial YAML to understand what YAML code is and to update yours [CLICK HERE FOR TUTORIAL YAML](https://psu-spatial.github.io/Geog364-2022/index_GEOG364-22_Tutorial_R.html#84_YAML_Code){target="_blank"}
...
**PART 3.** Customise.  Now customise! You can change the theme to something you like or add in other options.  Press knit to check it still works before you move on. Lots of themes in [Tutorial YAML](https://psu-spatial.github.io/Geog364-2022/index_GEOG364-22_Tutorial_R.html#84_YAML_Code){target="_blank"}
 
<br> 

**PART 4.** Now we want to update some *code chunk options*.  For each little grey code chunk there are a load of things we can do. For example we can make it hide the code and show the output (or vica-versa), change colour, or add captions. See [Tutorial 4.5 CODE CHUNKS]( https://psu-spatial.github.io/Geog364-2022/index_GEOG364-22_Tutorial_R.html#85_Code_chunks){target="_blank"} 
...
When you press knit, nothing will happen but later on in your labs when you knit, R will still show error messages, but hide friendly warnings. <br> See [Tutorial 4.5 CODE CHUNKS]( https://psu-spatial.github.io/Geog364-2022/index_GEOG364-22_Tutorial_R.html#85_Code_chunks){target="_blank"} 
...
*PART 6**  Save a new copy of your .Rmd file directly into your GEOG364 folder.  Now for future labs we can simply copy/paste this across.
