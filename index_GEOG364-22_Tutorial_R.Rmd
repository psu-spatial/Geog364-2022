...
Like there are many ways of accessing MSWord, there are many ways of
accessing R-Studio.

To make it easy to switch between them, we are going to be using
**"Projects"**.

These are self contained folders that contain all of your code, data and
output. This makes it easy to run R-studio and to move things around on
your computer. (e.g. if you move the whole folder, it all still runs).

For this course, I suggest one of two main options for accessing
R_studio. Each of the sections below is dedicated to getting up and
started and you can switch between them.
...
#### Signing up/Logging in {.unnumbered}
...
### Projects in R-Studio-CLOUD {.unnumbered}
...
#### How to create a project in R-Studio-CLOUD {.unnumbered}
...
### Downloading your work {.unnumbered}
...
### Uploading to the cloud {.unnumbered}
...
\*\*If you already have R and/or R-Studio, it is very important you
update BOTH of them to the most recent version. To do so, pretend you
are installing them from scratch. If you are worried this will affect
another class, chat with Dr Greatrex before starting out.

</p>

<br>

### Downloading R/R-Studio on a PC: {.unnumbered}

<p class="comment">

\*\*To make R-Studio run on your computer, you need to download TWO
things: [1] A programme that teaches your computer the ability to
"speak" in R and [2] The R studio software itself
...
### Downloading R/R-Studio on a Mac: {.unnumbered}
...
plots etc already loaded\*\*. It is always good to clear these before
you start a new analysis. To do this, click the little broom symbol in
...
## Using Projects on R-Studio Cloud {.unnumbered}

These are literally built in when you create a new project from the
workspace.

For each new lab, go back to the workspace and create a new one (see
above in the cloud section)

## Using Projects on R-Desktop {.unnumbered}
...
**A close analogy is your phone:** There are millions of apps available
from banking, to 50 different calendar apps. You don't have every app in
the world installed on your phone - and you don't have every app you
*do* download running at the same time. Instead you download the apps
that you think you will need (occasionally downloading a new one on the
fly) - and when you need to use an app, you click on it to open.
...
2.  You need to tell the computer you want to use the commands in that
    package (like clicking on the icon to start the app)
...
Look at the Packages tab next to the plot one, you can see which
packages/libraries are already pre-installed onto your computer.

If the package you want isn't listed then you need to download it from
the internet (as a one-off), like buying it from the app store

-   Click the INSTALL button in the Packages tab, then start typing the
    package name and it will show up (check the include dependencies
    box).

OR

-   IN THE CONSOLE! Run the `install.packages()` command on the package
    you want to download **with quotes around the package name** e.g.
...
Installing a package don't make the commands immediately available. For
that you need to load it (like clicking on an app). This can be done
...
If you have managed to install a package successfully, often nothing
...
You should now have R-Studio open and be inside an R project. If you're
having issues at this point or haven't managed to get to this step,
STOP! Ask an instructor for help.
...
through the video more slowly here:
...
## How to use the console {.unnumbered}

Remember that the aim of programming is to provide a language so you can
ask your computer to do complex tasks. The console window (see Figure
\@ref(fig:Tut_Fig1)) is like a phone call with your computer, where you
"speak" in R.
...
## Saving commands in "scripts" {.unnumbered}
...
WE WILL NORMALLY IGNORE THIS OPTION, but it's useful for a lot of
programming and zero writing
...
When you "run" the commands in the script, R simply copy/pastes the
commands over to the console. You run the commands by highlighting
(triple click to highlight a whole line), then press control-enter (or
command-enter on a mac). You can also copy/paste them accoss.

To open this you literally go to file, then new script.

This will add a new window - which is simply a notepad file you can
save. If you type your commands in there, you can save them. You run
them in the console by highlighting the text on each line and pressing
Ctrl-Enter (or command enter on a mac).

```{r, tut3fig4b, echo=FALSE, fig.cap = "Running commands in a basic script",fig.align='center'}
knitr::include_graphics('./Figures/pg_Tut3_basics_fig4b.png')
```

<br>

#### Creating a basic R script {.unnumbered}

For now, let's just create a basic R script. Go to the File menu at the
very top of the screen, then new file / new script.
...
You now have a space where you can save the work below if you wish. YOU
DO NOT NEED TO SUBMIT THIS. You are also welcome to simply enter the
...
When programming in R, someone often tells you about a "cool new
command". I want you to feel comfortable working out how to use them.
The R help files are also wonderful.

</p>

First, take a look here for all about help.
<https://www.r-project.org/help.html>

5.  Inside every R help file you will see this structure:
...
1.  If you don't already know, find the formal name and package name of
    the command

-   The only info we NEED to know about a new command is its name and
    the library/package/app it is stored in. <br>\
-   If you don't know, you can often google the command and look on the
    top left for the package and formal name. You often also find easy
    online tutorials.. <br>
...
knit button, there is a save button. Save it as something relevant
...
documents, which feels similar to traditional programs like Word. You
can find the full documentation at
...
specific one for your STAT462 Labs. For this section, I advise doing
everything in the standard view (E.g. don't press the little A symbol
for the pretty view)
